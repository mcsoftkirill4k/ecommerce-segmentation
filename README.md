...
**Результат: MSE 185, 3-е место** (Kaggle, 3 место на финальном лидерборде).

## Примеры работы

### Пример 1

![Пример сегментации 1](examples/download.png)

*Слева направо: исходное изображение, raw-предсказание модели (`Coarse`), результат после gamma/threshold-калибровки (`Refined`).*

### Пример 2

![Пример сегментации 2](examples/download-1.png)

*На этом примере хорошо видно, как постобработка убирает слабые паразитные области на фоне и делает маску чище.*

### Пример 3

![Пример сегментации 3](examples/download-2.png)

*Пример с более сложным силуэтом и полупрозрачными областями: после калибровки маска становится стабильнее, при этом основная форма объекта сохраняется.*
